...
def oh_cross_entropy(prediction,
                     ground_truth):
    """
    Function to calculate the cross entropy loss with softmax_cross_entropy_with_logits
    :param prediction: the logits (before softmax)
    :param ground_truth: the classification ground truth
    :return: the loss
    """
    ground_truth = tf.to_int64(ground_truth)
    loss = tf.nn.softmax_cross_entropy_with_logits(logits=prediction, labels=ground_truth)
    return loss