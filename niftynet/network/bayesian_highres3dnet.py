...
class HeteroHighRes3DNet(BaseNet):
    """
    implementation of single-task heteroscedastic network used in Bragman et al. Uncertainty in multi-task learning:
    ...
    def __init__(self,
        ...
        super(HeteroHighRes3DNet, self).__init__(
        ...
        self.main_network_layers = [
            ...
            {'name': 'res_1', 'n_features': 64, 'kernels': (3, 3), 'repeat': 3, 'dil': 1},
            {'name': 'res_2', 'n_features': 64, 'kernels': (3, 3), 'repeat': 3, 'dil': 2},
            {'name': 'res_3', 'n_features': 256, 'kernels': (3, 3), 'repeat': 3, 'dil': 4},
            {'name': 'conv_1', 'n_features': 512, 'kernel_size': 1}
        ]

        self.prediction = [
            {'name': 'pred_0', 'n_features': 128, 'kernel_size': 3},
            {'name': 'pred_1', 'n_features': 128, 'kernel_size': 3},
            {'name': 'pred_out', 'n_features': num_classes, 'kernel_size': 1}
        ]

        self.noise = [
            {'name': 'noise_0', 'n_features': 128, 'kernel_size': 3},
            {'name': 'noise_1', 'n_features': 128, 'kernel_size': 3},
    ...
    def layer_op(self, images, is_training=True, layer_id=-1, **unused_kwargs):
        ...
        # Create prediction network branching from representation network
        prediction_out, layer_instances = self.create_specific_branch(output_flow,
                                                                      is_training,
                                                                      layer_instances,
                                                                      self.prediction)

        # Create noise network branching from representation network
        noise_out, layer_instances = self.create_specific_branch(output_flow,
                                                                 is_training,
                                                                 layer_instances,
                                                                 self.noise)

        output = dict()
        output['prediction'] = prediction_out
        output['noise'] = noise_out
